...
class ProbUnitArray:
    """
    An array of independent probabilistic scalars.
    Flat stdlib array.array storage, row-major: row i = samples of element i.
    """
    ...
    def __init__(self, elements: Iterable[ProbUnitFloat]):
        ...
        for i, el in enumerate(elems):
            ...
            if el._n != self._n:
                raise ValueError(
                    f"Element {i} has {el._n} samples; expected {self._n}")
            f = el._unit.si_factor() / self._unit.si_factor()
            flat.extend(v * f for v in el._samples)
        self._data = _array.array('d', flat)

    @classmethod
    def _from_flat(cls, data: _array.array, unit, length: int, n: int) -> "ProbUnitArray":
    ...
    def _row(self, i: int) -> _array.array:
        return self._data[i * self._n: (i + 1) * self._n]

    def __len__(self) -> int: return self._len

    def __iter__(self) -> Iterator[ProbUnitFloat]:
        for i in range(self._len):
            yield ProbUnitFloat._from_raw(self._row(i), self._unit)
    ...
    def to_si(self) -> "ProbUnitArray":
        f  = self._unit.si_factor()
        cu = self._unit.to_si_compound()
        return ProbUnitArray._from_flat(
            _array.array('d', (v * f for v in self._data)), cu, self._len, self._n)

    def to(self, target) -> "ProbUnitArray":
    ...
    def _apply(self, o, op, result_unit) -> "ProbUnitArray":
        if isinstance(o, ProbUnitArray):
            ...
            out = _array.array('d')
            for i in range(self._len):
                out.extend(op(a, b[j]) for j, a in enumerate(self._row(i)))
    ...
    def _apply_mul(self, o, op) -> "ProbUnitArray":
    ...
    # ── Per-element statistics → UnitArray ───────────────────────────────────

    def means(self)   -> UnitArray:
        return UnitArray([sum(self._row(i)) / self._n   for i in range(self._len)], self._unit)
    def stds(self)    -> UnitArray:
        return UnitArray([statistics.stdev(self._row(i)) for i in range(self._len)], self._unit)
    def medians(self) -> UnitArray:
        return UnitArray([statistics.median(self._row(i))for i in range(self._len)], self._unit)

    def intervals(self, confidence: float = 0.95) -> list[tuple[UnitFloat, UnitFloat]]:
        tail   = (1 - confidence) / 2
        result = []
        for i in range(self._len):
    ...
    def __repr__(self):
        ...
        return (f"ProbUnitArray(means=[{', '.join(ms)}{dots}], "
                f"unit='{self._unit}', len={self._len}, n={self._n})")
