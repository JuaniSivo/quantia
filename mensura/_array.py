...
class UnitArray:
    """
    A homogeneous array of floats sharing one CompoundUnit.
    Backed by stdlib array.array('d').
    """
    ...
    def _elem(self, o, op) -> "UnitArray":
        ...
        if isinstance(o, UnitFloat):
            if not self._unit.is_compatible(o.unit):
                raise IncompatibleUnitsError(self._unit, o.unit)
    ...
    def _mul_div(self, o, op) -> "UnitArray":
    ...
    def sum(self)  -> UnitFloat: return UnitFloat(sum(self._data), self._unit)
    def mean(self) -> UnitFloat: return UnitFloat(sum(self._data)/len(self._data), self._unit)
    def min(self)  -> UnitFloat: return UnitFloat(min(self._data), self._unit)
    def max(self)  -> UnitFloat: return UnitFloat(max(self._data), self._unit)
    ...
    def __getitem__(self, idx):
        if isinstance(idx, slice):
            return UnitArray(self._data[idx], self._unit)
        return UnitFloat(self._data[idx], self._unit)

    def __setitem__(self, idx, val):
        if isinstance(val, UnitFloat):
            if not self._unit.is_compatible(val.unit):
                raise IncompatibleUnitsError(self._unit, val.unit)
            self._data[idx] = val.value * val.unit.si_factor() / self._unit.si_factor()
        else:
    ...
    def __repr__(self):
        preview = list(self._data[:6])
        dots = ", ..." if len(self._data) > 6 else ""
        return f"UnitArray([{', '.join(str(v) for v in preview)}{dots}], '{self._unit}')"
