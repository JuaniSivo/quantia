...
class TestTaggedRegistration:
    ...
    def test_m3_res_registered(self):
        assert get_unit("m3_res").to_si == pytest.approx(1.0, rel=1e-10)

    def test_m3_sc_registered(self):
        assert get_unit("m3_sc").to_si == pytest.approx(1.0, rel=1e-10)

    def test_cf_res_registered(self):
        assert get_unit("cf_res").to_si == pytest.approx(_SCF, rel=1e-10)

    def test_scf_registered(self):
        assert get_unit("scf").to_si == pytest.approx(_SCF, rel=1e-10)
    ...
    def test_Mcf_res_registered(self):
        assert get_unit("Mcf_res").to_si == pytest.approx(_SCF * 1_000, rel=1e-10)

    def test_Mscf_registered(self):
        assert get_unit("Mscf").to_si == pytest.approx(_SCF * 1_000, rel=1e-10)
...
class TestNonCancellation:
    ...
    def test_m3_res_div_m3_sc_not_dimensionless(self):
        gor = qu.Q(200.0, "m3_res") / qu.Q(1.0, "m3_sc")
        assert not gor.unit.is_dimensionless()

    def test_cf_res_div_scf_not_dimensionless(self):
    ...
    def test_Mcf_res_div_Mscf_not_dimensionless(self):
        gor = qu.Q(2.0, "Mcf_res") / qu.Q(1.0, "Mscf")
        assert not gor.unit.is_dimensionless()

    def test_m3_res_div_m3_res_IS_dimensionless(self):
        # Same tag divides normally — only different tags preserve label
        ratio = qu.Q(2.0, "m3_res") / qu.Q(1.0, "m3_res")
...
class TestGOR:
    ...
    def test_GOR_scfB_si_value(self):
        # 1000 cf_res / 1 STB
    ...
    def test_GOR_dry_gas_condensate(self):
        ...
        gor = qu.Q(50_000.0, "cf_res") / qu.Q(1.0, "STB")
...
class TestFVF:
    """
    Oil FVF (Bo): RB/STB or m3_res/m3_sc
    ...
    def test_Bo_m3_res_m3_sc_si_value(self):
        # Bo = 1.35 m3_res/m3_sc
        bo = qu.Q(1.35, "m3_res") / qu.Q(1.0, "m3_sc")
        assert bo.si_value() == pytest.approx(1.35, rel=1e-9)

    def test_Bo_RB_STB_equals_m3_res_m3_sc(self):
...
class TestOOIP:
    ...
    Where:
      Vp  = pore volume [m3_res]
      Sw  = water saturation [dimensionless]
      Bo  = oil FVF [m3_res/m3_sc]

    Result is in m3_sc → convert to bbl
    """

    def test_OOIP_basic(self):
        # Vp=1e6 m3_res, Sw=0.25, Bo=1.2
    ...
    def test_OOIP_higher_Bo_gives_lower_OOIP(self):
        ...
        Vp = qu.Q(1_000_000.0, "m3_res")
        Sw = 0.25
        Bo_low  = qu.Q(1.1, "m3_res") / qu.Q(1.0, "m3_sc")
        Bo_high = qu.Q(1.5, "m3_res") / qu.Q(1.0, "m3_sc")
...
class TestOGIP:
    ...
    Where Bg = gas FVF [m³_res/scfd] — typically very small number.
    For dry gas reservoirs, often expressed as m3_res/m3_sc.
    ...
    def test_OGIP_m3_basis(self):
        ...
        Vp   = qu.Q(1e8, "m3_res")
        Sw   = 0.30
        Bg   = qu.Q(0.005, "m3_res") / qu.Q(1.0, "m3_sc")
        ogip = Vp * (1 - Sw) / Bg
...
class TestProbTagged:
    ...
    def test_prob_OOIP_mean_matches_deterministic(self):
        ...
        with qu.config(seed=42, n_samples=5000):
            Bo_prob = qu.ProbUnitFloat.normal(1.2, 0.05, "m3_res")

        Bo_st = qu.Q(1.0, "m3_sc")
        ...
        det_ooip = 1_000_000.0 * 0.75 / 1.2  # m3_sc
    ...
    def test_prob_OOIP_std_positive(self):
        ...
        with qu.config(seed=0, n_samples=2000):
            Bo_prob = qu.ProbUnitFloat.uniform(1.1, 1.5, "m3_res")

        Bo = Bo_prob / qu.Q(1.0, "m3_sc")
        ooip = Vp * (1 - Sw) / Bo
    ...
    def test_prob_GOR_mean(self):
        ...
        with qu.config(seed=1, n_samples=3000):
            gas = qu.ProbUnitFloat.normal(100.0, 5.0, "m3_res")
