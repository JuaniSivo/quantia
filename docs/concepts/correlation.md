...
In a reservoir, porosity and permeability tend to move together —
higher porosities tend to have higher permeabilitites due to bigger
throuts in the rock connecting pores. If you sample them independently,
you miss scenarios where both are simultaneously high (optimistic) or
simultaneously low (pessimistic).
...
with qu.config(n_samples=3000, seed=0):
    ...
    k = src.draw(1, 'uniform',    'mD',  low=0.60, high=0.90)
```

`rho=0.7` means a Pearson correlation of 0.7 between `phi` and `k`. High
porosity samples are more likely to coincide with high permeability
samples.
...
with qu.config(n_samples=3000, seed=0):
    ...
    k         = src.draw(0, 'triangular', 'mD',  low=10, mode=15, high=22)
...
# RuntimeError: Slot 0 already drawn. Each slot can only be used once.
```